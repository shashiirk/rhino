...
import { makePositive, toTitleCase } from '../services/utils';
...
const FormEditTemplate = (props) => {
  ...
  const { item } = useContext(EditItemContext);

  const positiveAmount = makePositive(item.amount);

  const [width, setWidth] = useState(
    Number.isInteger(positiveAmount)
      ? positiveAmount.toString().length
      : positiveAmount.toString().length - 1
  );
  const [amount, setAmount] = useState(positiveAmount);
  ...
  const submitFormHandler = (ev) => {
    ...
    if (isFormValid) {
      if (props.type === 'income') {
        ...
        updateDoc(doc(db, authCtx.user.uid, item.id), updatedItem).catch(
          (error) => {
            console.log(error);
          }
        );
  ...
  return (
    <Div className={themeCtx.dark ? 'dark' : ''}>
      ...
      <form onSubmit={submitFormHandler}>
        <h2 className="title">
          {props.type === 'income' ? 'Edit Income' : 'Edit Expenses'}
        </h2>
        ...
        <div className="form-control">
          <PickDate onChange={setDateHandler} selected={date} />
        </div>
        <button
          type="submit"
